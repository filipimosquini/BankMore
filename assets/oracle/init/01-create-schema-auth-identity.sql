...
BEGIN
  SELECT COUNT(*)
    INTO v_user_count
    FROM dba_users
   WHERE username = 'AUTH_IDENTITY';

  IF v_user_count = 0 THEN
    EXECUTE IMMEDIATE q'[
      CREATE USER AUTH_IDENTITY IDENTIFIED BY auth123
    ...
    -- Permissões básicas
    EXECUTE IMMEDIATE 'GRANT CONNECT, RESOURCE TO AUTH_IDENTITY';

    -- Permissões necessárias para EF Core / Identity
    EXECUTE IMMEDIATE 'GRANT CREATE TABLE, CREATE VIEW, CREATE SEQUENCE, CREATE PROCEDURE, CREATE TRIGGER TO AUTH_IDENTITY';
  END IF;
END;
/
-- =========================================================
