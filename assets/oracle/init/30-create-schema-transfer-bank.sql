...
BEGIN
  SELECT COUNT(*)
    INTO v_user_count
    FROM dba_users
   WHERE username = 'TRANSFER_BANK';

  IF v_user_count = 0 THEN
    EXECUTE IMMEDIATE q'[
      CREATE USER TRANSFER_BANK IDENTIFIED BY auth123
    ...
    -- Permissões básicas
    EXECUTE IMMEDIATE 'GRANT CONNECT, RESOURCE TO TRANSFER_BANK';

    -- Permissões necessárias para EF Core / Identity
    EXECUTE IMMEDIATE 'GRANT CREATE TABLE, CREATE VIEW, CREATE SEQUENCE, CREATE PROCEDURE, CREATE TRIGGER TO TRANSFER_BANK';
  END IF;
END;
/
-- =========================================================
