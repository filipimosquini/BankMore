...
BEGIN
  SELECT COUNT(*)
    INTO v_user_count
    FROM dba_users
   WHERE username = 'FEE_BANK';

  IF v_user_count = 0 THEN
    EXECUTE IMMEDIATE q'[
      CREATE USER FEE_BANK IDENTIFIED BY auth123
    ...
    -- Permissões básicas
    EXECUTE IMMEDIATE 'GRANT CONNECT, RESOURCE TO FEE_BANK';

    -- Permissões necessárias para EF Core / Identity
    EXECUTE IMMEDIATE 'GRANT CREATE TABLE, CREATE VIEW, CREATE SEQUENCE, CREATE PROCEDURE, CREATE TRIGGER TO FEE_BANK';
  END IF;
END;
/
-- =========================================================
