...
using Account.Application.Movement.Commands.CreateMovementByHolder;
...
public class MovementController(ILoggerFactory loggerFactory, IMediator mediatorService) : BaseController<MovementController>(loggerFactory, mediatorService)
{
    ...
    /// <summary>
    /// api/movements/holder.
    /// </summary>
    /// <remarks>
    /// <p>
    /// <b> Description: </b><br />
    /// This method create a movement by holder account. <br />
    /// </p>
    /// <p>
    /// <b> Requirements: </b><br />
    /// Is required to be authenticated. <br />
    ...
    [HttpPost("holder")]
    [Authorize]
    [SwaggerRequestExample(typeof(CreateMovementByHolderRequest), typeof(CreateMovementByHolderRequestExample))]
    [ProducesResponseType((int)HttpStatusCode.NoContent)]
    public async Task<IActionResult> CreateMovementByHolderAsync([FromBody] CreateMovementByHolderRequest request, [FromHeader(Name = "Idempotency-Key")] Guid requestId)
        => await ExecuteAsync(async () => await _mediatorService.Send(new CreateMovementByHolderCommand(requestId, request.Amount, request.MovementType, UserId)), HttpStatusCode.NoContent);
